...
END
  ...
  volumes+=($(cat <<-END
    --volume $ssl_cert:/usr/share/opensearch/config/certs/testnode.crt
    --volume $ssl_key:/usr/share/opensearch/config/certs/testnode.key
    --volume $ssl_ca:/usr/share/opensearch/config/certs/ca.crt
END
))
fi
...
do
   docker pull docker.opensearch.co/opensearch/"$opensearch_container" && break
