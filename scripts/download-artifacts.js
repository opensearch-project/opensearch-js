...
async function resolve(version, hash) {
  ...
  if (hash != null) {
    ...
    const zipKey = Object.keys(build.projects.packages).find(
      (key) => key.startsWith('rest-resources-zip-') && key.endsWith('.zip')
    );
    return {
  ...
  const zipKey = Object.keys(lastBuild.projects.packages).find(
    (key) => key.startsWith('rest-resources-zip-') && key.endsWith('.zip')
  );
  return {
