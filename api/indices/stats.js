 ...
 * @param {boolean} [params.forbid_closed_indices=true] - If true, statistics are not collected from closed indices.
...
function statsFunc(params, options, callback) {
  ...
  const path = ['/', index, '/_stats/', metric].filter(c => c).join('').replace('//', '/');
  const method = 'GET';
  body = body || '';
