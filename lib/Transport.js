    ...
    this.request({
      ...
      if (err) {
        ...
        if (err.statusCode === 401 || err.statusCode === 403) {
          ...
          process.emitWarning('The client is unable to verify that the server is OpenSearch due to security privileges on the server side. Some functionality may not be compatible if the server is running an unsupported product.')
        ...
        if (major !== 7) {
          debug('Invalid OpenSearch distribution')
          return compatibleCheckEmitter.emit('compatible-check', false)
        }
