  ...
  request (params, options, callback) {
    ...
    if (this[kCompatibleCheck] === 0 || this[kCompatibleCheck] === 1) {
      ...
      if (params.method === 'GET' && params.path === '/') {
        ...
        productCheckEmitter.once('compatible-check', status => {
          if (status === false) {
    ...
    // the compatible check is finished and it's not Elasticsearch
    } else if (this[kCompatibleCheck] === 3) {
      const err = new ProductNotSupportedError(result)
      this.emit('request', err, result)
      process.nextTick(callback, err, result)
    // the compatible check finished and it's Elasticsearch
    } else {
      prepareRequest()
    }
    ...
    this.request({
      ...
      if (err) {
        ...
        if (err.statusCode === 401 || err.statusCode === 403) {
          ...
          productCheckEmitter.emit('compatible-check', true)
        } else {
          this[kCompatibleCheck] = 0
          productCheckEmitter.emit('compatible-check', false)
        }
      } else {
        ...
        if (result.body.version == null || typeof result.body.version.number !== 'string') {
          ...
          return productCheckEmitter.emit('compatible-check', false)
        }
        ...
        if (major !== 7) {
          debug('Invalid Elasticsearch distribution')
          return productCheckEmitter.emit('compatible-check', false)
        }
