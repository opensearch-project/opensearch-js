 ...
 * <br/> See Also: {@link undefined - ltr.search_models}
 *
 * @memberOf API-Ltr
 *
 * @param {object} [params]
 * @param {number} [params.from=0] - The offset from the first result (for pagination).
 * @param {string} [params.prefix] - A name prefix to filter models by.
 * @param {number} [params.size=20] - The number of models to return.
...
function searchModelsFunc(params, options, callback) {
  ...
  const path = ['/_ltr', store, '_model'].filter(c => c != null).join('/');
  const method = 'GET';
  body = body || '';
