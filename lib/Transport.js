...
class Transport {
  ...
  request (params, options, callback) {
    ...
    // still need to check the compatibility or waiting for the check to finish
    if (this[kCompatibleCheck] === 0 || this[kCompatibleCheck] === 1) {
      // let pass info requests
      if (params.method === 'GET' && params.path === '/') {
        prepareRequest()
      } else {
        // wait for compatible check to finish
        compatibleCheckEmitter.once('compatible-check', status => {
          if (status === false) {
            const err = new NotCompatibleError(result)
            this.emit('request', err, result)
            process.nextTick(callback, err, result)
          } else {
            prepareRequest()
        ...
        // the very first request triggers the compatible check
        if (this[kCompatibleCheck] === 0) {
          this.checkCompatibleInfo()
        }
      }
      // the compatible check is finished and it's not OpenSearch
    } else if (this[kCompatibleCheck] === 3) {
      const err = new NotCompatibleError(result)
      this.emit('request', err, result)
      process.nextTick(callback, err, result)
      // the compatible check finished and it's OpenSearch
    } else {
      prepareRequest()
    }
    ...
    this.request({
      ...
      if (err) {
        ...
        if (err.statusCode === 401 || err.statusCode === 403) {
          ...
          process.emitWarning('The client is unable to verify that the server is OpenSearchdue to security privileges on the server side. Some functionality may not be compatible if the server is running an unsupported product.')
        ...
        if (result.body.version == null || typeof result.body.version.number !== 'string') {
          debug('Can\'t access OpenSearchversion')
        ...
        if (major !== 7) {
          debug('Invalid OpenSearchdistribution')
          return compatibleCheckEmitter.emit('compatible-check', false)
        }

        debug('Valid OpenSearchdistribution')
        this[kCompatibleCheck] = 2
        compatibleCheckEmitter.emit('compatible-check', true)
      }
    })
