 ...
 * @param {integer} [params.batched_reduce_size=512] - The number of shard results that should be reduced at once on the coordinating node. This value should be used as a protection mechanism to reduce the memory overhead per search request if the potential number of shards in the request can be large.
 ...
 * @param {integer} [params.from=0] - Starting offset.
 ...
 * @param {integer} [params.max_concurrent_shard_requests=5] - The number of concurrent shard requests per node this search executes concurrently. This value should be used to limit the impact of the search on the cluster in order to limit the number of concurrent shard requests.
 * @param {integer} [params.pre_filter_shard_size] - Threshold that enforces a pre-filter round-trip to prefilter search shards based on query rewriting if the number of shards the search request expands to exceeds the threshold. This filter round-trip can limit the number of shards significantly if for instance a shard can not match any documents based on its rewrite method ie. if date filters are mandatory to match but the shard bounds and the query are disjoint.
 ...
 * @param {integer} [params.size=10] - Number of hits to return.
 ...
 * @param {integer} [params.suggest_size] - How many suggestions to return in response.
 * @param {string} [params.suggest_text] - The source text for which the suggestions should be returned.
 * @param {integer} [params.terminate_after] - The maximum number of documents to collect for each shard, upon reaching which the query execution will terminate early.
