...
function commonParameters(spec) {
  ...
  Object.keys(spec.params).forEach((key) => {
    const name =
      isSnakeCased(key) && key !== camelify(key)
        ? '`' + key + '` or `' + camelify(key) + '`'
        : '`' + key + '`';
...
function generateApiDoc(spec) {
  ...
  const documentationUrl =
    spec[name].documentation && spec[name].documentation.url
  ...
  const stability = spec[name].stability === 'stable' ? '' : `*Stability:* ${spec[name].stability}`;
  ...
  if (params.length !== 0) {
    ...
    doc += params.reduce((acc, val) => {
      const name =
        isSnakeCased(val.name) && val.name !== camelify(val.name)
...
function fixLink(name, str) {
   ...
   * the json files in the OpenSearch repo are a bit of a mess. */
  str = str.replace(
    /^.+guide\/en\/opensearch\/reference\/[^/]+\/([^./]*\.html(?:#.+)?)$/,
    '{ref}/$1'
  );
